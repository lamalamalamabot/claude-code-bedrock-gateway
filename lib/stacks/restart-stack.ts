...
/**
 * Daily rolling restart of the LiteLLM Fargate service at 04:00 KST.
 *
 * The service shows gradual performance degradation over long uptimes (root
 * cause unidentified — likely a slow leak). A daily rolling restart clears
 * it. We call ecs:UpdateService with forceNewDeployment=true, which spins up
 * fresh tasks, waits for health checks, then drains the old ones —
 * zero-downtime, no Lambda required. The service already has a deployment
 * circuit breaker with rollback, so a bad restart self-heals.
 */
